...
/*	CodeCanyon Widget Class */

class PB_CodeCanyon_Widget extends WP_Widget {

	var $cc_cats; //CodeCanyon items categories
	var $exclude; //Wheter to exclude items or not
	var $defaults;

	function __construct() {
		$widget_ops = array( 'classname' => 'pb_codecanyon_widget', 'description' => __( 'Display CodeCanyon items with this widget', 'affiliate-codecanyon-widget' ) );
		$control_ops = array( 'id_base' => 'pb_codecanyon_widget' );
		parent::__construct( 'pb_codecanyon_widget', __( 'Affiliate CodeCanyon Widget', 'affiliate-codecanyon-widget' ), $widget_ops, $control_ops );

		$this->cc_cats = array(
			array( 'name' => 'wordpress', 'title' => 'WordPress' ),
			array( 'name' => 'php-scripts', 'title' => 'PHP Scripts' ),
			array( 'name' => 'javascript', 'title' => 'JavaScript' ),
			array( 'name' => 'css', 'title' => 'CSS' ),
			array( 'name' => 'ecommerce', 'title' => 'eCommerce' ),
			array( 'name' => 'mobile', 'title' => 'Mobile' ),
			array( 'name' => 'html5', 'title' => 'HTML5' ),
			array( 'name' => 'skins', 'title' => 'Skins' ),
			array( 'name' => 'plugins', 'title' => 'Plugins' ),
			array( 'name' => 'apps', 'title' => 'Apps' )
		);
		...
		$this->defaults = array(
			...
			'orderby' => 'uploaded_on',
			'ref' => 'pluginsbay',
			'more_link_url' => 'https://codecanyon.net/user/themepunch/portfolio?ref=pluginsbay',
			'more_link_txt' => __( 'View more', 'affiliate-codecanyon-widget' ),
			'order' => 'desc',
			'target' => '_blank',
			'exclude' => '',
			'nofollow' => 0,
		...
		$this->defaults = apply_filters( 'pb_cc_widget_modify_defaults', $this->defaults );
...
?>
		<ul class="pb_codecanyon_widget_ul">
		...
		<?php if ( !empty( $instance['more_link_url'] ) ): ?>
			 <?php $more_text = isset( $instance['more_link_txt'] ) && !empty( $instance['more_link_txt'] ) ? $instance['more_link_txt'] : __( 'View more', 'affiliate-codecanyon-widget' ); ?>
			  <p class="pb_read_more"><a href="<?php echo esc_url( $instance['more_link_url'] ); ?>" target="_blank" class="more" <?php echo $nofollow; ?>><?php echo  esc_html( $more_text ); ?></a></p>
			 <?php endif; ?>
		<?php endif; ?>
	...
	function form( $instance ) {
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'title' ); ?>">
		        <?php _e( 'Title', 'affiliate-codecanyon-widget' ); ?>:</label>
		    <input id="<?php echo $this->get_field_id( 'title' ); ?>" type="text" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" class="widefat" />
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'description' ); ?>">
		        <?php _e( 'Description', 'affiliate-codecanyon-widget' ); ?>:</label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'token' ); ?>">
		        <?php _e( 'Envato API token', 'affiliate-codecanyon-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'It is highly recommended to provide your Envato API token as using the service without it will be discontinued soon.', 'affiliate-codecanyon-widget' ); ?></small>
		    <small class="howto"><?php echo '<a href="https://build.envato.com/create-token/" target="_blank">'.__( 'Generate token', 'affiliate-codecanyon-widget' ).'</a>'; ?></small>
		    
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'num_items' ); ?>">
		        <?php _e( 'Number of items to show', 'affiliate-codecanyon-widget' ); ?>:</label>
		...
		<p>
		    <label>
		        <?php _e( 'Select items from', 'affiliate-codecanyon-widget' ); ?>:</label><br />
		    <input id="<?php echo $this->get_field_id( 'select_from_popular' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'items_from' ); ?>" value="popular" <?php checked( $instance['items_from'], 'popular' );?> class="affiliate-codecanyon-widget-type" /> <label for="<?php echo $this->get_field_id( 'select_from_popular' ); ?>">
		        <?php _e( 'Popular Items (WordPress Only)', 'affiliate-codecanyon-widget' ); ?></label><br />
		    <input id="<?php echo $this->get_field_id( 'select_from_latest' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'items_from' ); ?>" value="latest" <?php checked( $instance['items_from'], 'latest' );?> class="affiliate-codecanyon-widget-type" /> <label for="<?php echo $this->get_field_id( 'select_from_latest' ); ?>">
		        <?php _e( 'Latest Items', 'affiliate-codecanyon-widget' ); ?></label><br />
		    <input id="<?php echo $this->get_field_id( 'select_from_user' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'items_from' ); ?>" value="user" <?php checked( $instance['items_from'], 'user' );?> class="affiliate-codecanyon-widget-type" /> <label for="<?php echo $this->get_field_id( 'select_from_user' ); ?>">
		        <?php _e( 'Specific User(s)', 'affiliate-codecanyon-widget' ); ?></label>
		</p>
		<p class="affiliate-codecanyon-widget-user" <?php echo $this->selected( $instance['items_from'], 'user', 'style="display:none;"', true ); ?>>
		    <label for="<?php echo $this->get_field_id( 'user' ); ?>">
		        <?php _e( 'CodeCanyon username(s)', 'affiliate-codecanyon-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'For multiple users, separate by comma: i.e. user1,user2,user3', 'affiliate-codecanyon-widget' ); ?></small>
		</p>
		<p class="affiliate-codecanyon-widget-cat" <?php echo $this->selected( $instance['items_from'], 'popular', 'style="display:none;"'); ?>>
		    <label>
		        <?php _e( 'Item categories to show', 'affiliate-codecanyon-widget' ); ?>:</label><br />
		    <?php foreach ( $this->cc_cats as $cat ) : ?>
		...
		<p class="affiliate-codecanyon-widget-order" <?php echo $this->selected( $instance['items_from'], 'user', 'style="display:none;"', true ); ?>>
		    <label>
		        <?php _e( 'Order by', 'affiliate-codecanyon-widget' ); ?>:</label>
		...
		<p class="affiliate-codecanyon-widget-order" <?php echo $this->selected( $instance['items_from'], 'user', 'style="display:none;"', true ); ?>>
		    <input id="<?php echo $this->get_field_id( 'order_asc' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'order' ); ?>" value="asc" <?php checked( $instance['order'], 'asc' );?> /> <label for="<?php echo $this->get_field_id( 'order_asc' ); ?>">
		        <?php _e( 'Ascending', 'affiliate-codecanyon-widget' ); ?></label>
		    <input id="<?php echo $this->get_field_id( 'order_desc' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'order' ); ?>" value="desc" <?php checked( $instance['order'], 'desc' );?> /> <label for="<?php echo $this->get_field_id( 'order_desc' ); ?>">
		        <?php _e( 'Descending', 'affiliate-codecanyon-widget' ); ?></label>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'exclude' ); ?>">
		        <?php _e( 'Exclude item(s)', 'affiliate-codecanyon-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'Specify item ID to exclude specific item (separate by comma for multiple items): i.e. 8134834,7184572', 'affiliate-codecanyon-widget' ); ?></small>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'ref' ); ?>">
		        <?php _e( 'Referral user', 'affiliate-codecanyon-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'Specify username if you want to use items as CodeCanyon affiliate links', 'affiliate-codecanyon-widget' ); ?></small>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'more_link_url' ); ?>">
		        <?php _e( 'More link URL', 'affiliate-codecanyon-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'Specify URL if you want to show "more" link under the items list', 'affiliate-codecanyon-widget' ); ?></small>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'more_link_txt' ); ?>">
		        <?php _e( 'More link text', 'affiliate-codecanyon-widget' ); ?>:</label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'target' ); ?>">
		        <?php _e( 'Open items in', 'affiliate-codecanyon-widget' ); ?>: </label>
		    <select id="<?php echo $this->get_field_id( 'target' ); ?>" name="<?php echo $this->get_field_name( 'target' ); ?>">
		        <option value="_blank" <?php selected( '_blank' , $instance['target'] ); ?>>
		            <?php _e( 'New Window', 'affiliate-codecanyon-widget' ); ?>
		        </option>
		        <option value="_self" <?php selected( '_self' , $instance['target'] ); ?>>
		            <?php _e( 'Same Window', 'affiliate-codecanyon-widget' ); ?>
		        </option>
		    </select>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'nofollow' ); ?>"><input id="<?php echo $this->get_field_id( 'nofollow' ); ?>" type="checkbox" name="<?php echo $this->get_field_name( 'nofollow' ); ?>" value="1" <?php checked( $instance['nofollow'], 1 ); ?> class="widefat" />
		        <?php _e( 'Add rel="nofollow" to item links', 'affiliate-codecanyon-widget' ); ?>
		    </label>
		</p>
	...
	function get_items_from_users( $users = array(), $type = array( 'wordpress' ) ) {
		...
		foreach ( $users as $user ) {
			...
			if ( empty( $cached ) ) {
				...
				$query = 'new-files-from-user:'.$user.',codecanyon.json';
			...
			$type_check = count( $type ) == count( $this->cc_cats ) ? false : true;
	...
	function get_popular_items( $type = array( 'wordpress' ) ) {
		...
		$type = array( 'wordpress' ); //Hardcoded to WordPres only, due to current CodeCanyon API limitations
	...
	function get_latest_items( $types = array( 'wordpress' ) ) {
		...
		foreach ( $types as $type ) {
			...
			if ( empty( $cached ) ) {
				...
				$query = 'new-files:codecanyon,'.$type.'.json';
	...
	function enqueue_styles() {
		wp_register_style( 'pluginsbay-codecanyon-widget', PB_CC_PLUGIN_URI . 'css/style.css', false, PB_CC_WIDGET_VER );
		wp_enqueue_style( 'pluginsbay-codecanyon-widget' );
	}
	...
	function enqueue_scripts() {
		...
		if( $pagenow == 'widgets.php' ){
			wp_enqueue_script( 'pluginsbay-codecanyon-widget', PB_CC_PLUGIN_URI . 'js/admin.js', array('jquery') , PB_CC_WIDGET_VER );
		}	
	}
